...
export default function KaynakAnaliziPage() {
  ...
  const bestSource = stats.length ? stats.reduce((a, b) => (a.totalAmount > b.totalAmount ? a : b)) : null;
      ...
      case "bar":
        return (
          <BarChart {...chartProps} barCategoryGap={30} barGap={4}>
            ...
            <XAxis dataKey="sourceName" angle={-45} textAnchor="end" height={120} interval={0} tick={{ fill: "#555", fontSize: 12 }} />
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-10 lg:p-12">
      <header className="mb-8 text-center">
        <h1 className="text-3xl font-extrabold" style={{ color: CORPORATE_COLOR }}>Kaynak/Kanal Analizi</h1>
        <p className="mt-2 text-gray-700">Anlaşmaların hangi kaynaklardan geldiği ve getirisini analiz edin.</p>
      </header>
      ...
      <div className="flex flex-wrap gap-6 mb-8 justify-center">
        ...
        {bestSource && (
          <div className="bg-white rounded-xl shadow p-6 min-w-[200px] text-center">
            <div className="text-gray-500 mb-2">En Çok Getiren Kanal</div>
            <div className="text-lg font-bold" style={{ color: CORPORATE_COLOR }}>{bestSource.sourceName}</div>
            <div className="text-md" style={{ color: CORPORATE_COLOR_LIGHT }}>₺{Number(bestSource.totalAmount).toLocaleString("tr-TR")}</div>
          </div>
        )}
      </div>
