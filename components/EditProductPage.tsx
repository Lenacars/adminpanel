  ...
  return (
    <div className="p-6 space-y-10 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded shadow">
        ...
        {[{ label: "Yakıt Türü", name: "yakit_turu", options: YAKIT_OPTIONS }, { label: "Vites", name: "vites", options: VITES_OPTIONS }, { label: "Marka", name: "brand", options: MARKA_OPTIONS }, { label: "Segment", name: "segment", options: SEGMENT_OPTIONS }, { label: "Gövde Tipi", name: "bodyType", options: BODYTYPE_OPTIONS }, { label: "Durum", name: "durum", options: DURUM_OPTIONS }].map(({ label, name, options }) => (
          <div key={name}>
            <label>{label}</label>
      ...
      <div className="bg-white p-6 rounded shadow space-y-4">
        ...
        <textarea className="w-full border p-2 rounded" rows={4} value={kisaAciklama} onChange={(e) => setKisaAciklama(e.target.value)} />
        <label className="block mb-2">Detaylı Açıklama</label>
        <textarea className="w-full border p-2 rounded" rows={8} value={aciklama} onChange={(e) => setAciklama(e.target.value)} />
      </div>
