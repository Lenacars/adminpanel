...
export default function Sidebar() {
  ...
  if (isSidebarLoading) {
    return (
      <aside className="h-screen bg-[#6A3C96] w-64 p-4 flex items-center justify-center">
    ...
    return items.map((item) => {
      ...
      if (item.children) {
        ...
        return (
          <div key={item.label}>
            ...
            >
              <div className="mt-1 pl-9 space-y-1 py-1"> {/* Çocuklar için girinti ve dikey boşluk */}
      ...
      return (
        <Link
          ...
          `}
        >
          {currentItemIsActive && !item.highlight && ( // Highlight aktifse kendi stilini korusun
            <span className="absolute left-0 top-1/2 transform -translate-y-1/2 h-5 w-[3px] bg-purple-300 rounded-r-full"></span>
          )}
          {item.icon && (
            <span className="w-6 h-6 mr-3 flex items-center justify-center text-purple-200 group-hover:text-slate-50 transition-colors">
  ...
  return (
    <aside className="h-screen bg-[#6A3C96] w-64 text-slate-100 flex flex-col justify-between shadow-xl">
      {/* Scrollable Alan Başlangıcı */}
      <div className="flex flex-col flex-grow overflow-y-auto sidebar-scrollable pr-0.5"> {/* pr-0.5 scrollbar için küçük bir boşluk */}
        <div className="p-4"> {/* Üst kısım için genel padding */}
          ...
          {adSoyad && rol && (
            <div className="bg-[#5b3482]/70 rounded-lg p-3.5 mb-6 shadow-md"> {/* Arka planı biraz daha transparan yaptık */}
        ...
        <nav className="space-y-1.5 px-4 flex-grow"> {/* Menü öğeleri için padding ve dikey boşluk */}
          {renderMenuItems(unifiedMenuItems)}
          
          {extraMenuItems.length > 0 && (
            <div className="mt-6 pt-6 border-t border-purple-500/40"> {/* Ayırıcı çizgi opaklığı azaltıldı */}
      ...
      <div className="p-4 mt-auto border-t border-purple-500/40"> {/* Çıkış butonu için üst çizgi ve padding */}
        <button
          onClick={async () => {
...
/*
  globals.css veya ana stil dosyanıza ekleyin:
